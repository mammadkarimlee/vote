...
const LOGIN_EMAIL_DOMAIN = process.env.LOGIN_EMAIL_DOMAIN || process.env.VITE_LOGIN_EMAIL_DOMAIN || 'vote.local'
const PORT = Number(process.env.PROVISION_API_PORT || 8787)
const allowedOrigins = (process.env.PROVISION_ALLOWED_ORIGINS || 'http://localhost:5173')
  .split(',')
...
const normalizeLoginPart = (value) => value
  .split('')
...
app.post('/provision-user', async (req, res) => {
  try {
    ...
    if (mode !== 'login' && mode !== 'email') return respondError(res, 400, 'Invalid mode')

    const isSuperAdmin = actor.role === 'superadmin'
    const isBranchStaff = actor.role === 'branch_admin' || actor.role === 'moderator'
    if (!isSuperAdmin && !isBranchStaff) return respondError(res, 403, 'Forbidden')
    ...
    if (createError || !created.user) {
      return respondError(res, 400, createError?.message || 'Auth user not created')
    }
    ...
    if (role === 'student') {
      ...
      if (studentError) {
        if (mode === 'login') {
          await supabase.from('usernames').delete().eq('org_id', orgId).eq('login', login)
        }
        await supabase.from('users').delete().eq('id', uid)
        await supabase.auth.admin.deleteUser(uid)
    ...
    return respondError(res, 500, error instanceof Error ? error.message : 'Unexpected error')
  }
})
