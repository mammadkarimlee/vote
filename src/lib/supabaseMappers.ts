...
export const mapPkpdFinalReviewRow = (row: Row): PkpdFinalReviewDoc => ({
	cycleId: row.cycle_id,
	branchId: row.branch_id,
	teacherId: row.teacher_id,
	...
	updatedAt: row.updated_at ?? null,
	isManualEdited: row.is_manual_edited ?? false,
	createdAt: row.created_at ?? null,
});
...
export const mapPkpdTeacherSummaryRow = (row: Row): PkpdTeacherSummaryDoc => ({
	teacherId: row.teacher_id,
	branchId: row.branch_id ?? null,
	...
	isBiqTeacher: row.is_biq_teacher !== false,
	studentAvg: nullableNumber(row.student_avg),
	managementAvg: nullableNumber(row.management_avg),
	selfAvg: nullableNumber(row.self_avg),
	selfDeclaredScore: nullableNumber(row.self_declared_score),
	academicIndicator: nullableNumber(row.academic_indicator),
	teacherCriteriaTotal: nullableNumber(row.teacher_criteria_total),
	hrEvaluationScore: nullableNumber(row.hr_evaluation_score),
	biqAvg: nullableNumber(row.biq_avg),
	computedBiqAvg: nullableNumber(row.computed_biq_avg),
	manualBiqAvg: nullableNumber(row.manual_biq_avg),
	biqAverageSource: row.biq_average_source ?? "none",
	studentWeightedScore: nullableNumber(row.student_weighted_score),
	managementWeightedScore: nullableNumber(row.management_weighted_score),
	leadershipSubmittedCount: Number(row.leadership_submitted_count ?? 0),
	leadershipEligibleCount: Number(row.leadership_eligible_count ?? 0),
	leadershipComplete: row.leadership_complete ?? false,
	leadershipOverridden: row.leadership_overridden ?? false,
	branchManagerSubmitted: row.branch_manager_submitted ?? false,
	deputySubmitted: row.deputy_submitted ?? false,
	departmentHeadSubmitted: row.department_head_submitted ?? false,
	branchManagerEligible: row.branch_manager_eligible ?? false,
	deputyEligible: row.deputy_eligible ?? false,
	departmentHeadEligible: row.department_head_eligible ?? false,
	selfWeightedScore: nullableNumber(row.self_weighted_score),
	biqWeightedScore: nullableNumber(row.biq_weighted_score),
	examScore: nullableNumber(row.exam_score),
	portfolioScore: nullableNumber(row.portfolio_score),
	bonusScore: Number(row.bonus_score ?? 0),
	currentEnteredScore: Number(row.current_entered_score ?? 0),
	isComplete: row.is_complete ?? false,
	...
	studentClassScores: Array.isArray(row.student_class_scores)
		? row.student_class_scores.map((item: Row) => ({
				groupId: item.groupId ?? "no-group",
				groupName: item.groupName ?? item.groupId ?? "no-group",
				avg: Number(item.avg ?? 0),
				submissionCount: Number(item.submissionCount ?? 0),
			}))
		: [],
	managementCount: Number(row.management_count ?? 0),
