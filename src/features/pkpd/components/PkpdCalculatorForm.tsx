...
const fieldMeta = {
  ...
  leadership: { min: 0, max: 10, label: 'Rəhbərlik dəyərləndirməsi (0–10)', weight: 10 },
...
const parseField = (value: string, min: number, max: number): FieldState => {
  ...
  if (Number.isNaN(parsed)) return { value, error: 'Yalnız rəqəm daxil edin', numeric: 0 }
  if (parsed < min || parsed > max) return { value, error: `Aralıq ${min}–${max} olmalıdır`, numeric: parsed }
...
export const PkpdCalculatorForm = () => {
  ...
  const values = useMemo(() => {
    const parsed: Record<FieldKey, FieldState> = {
      ...
      survey: parseField(fields.survey, fieldMeta.survey.min, fieldMeta.survey.max),
      self: parseField(fields.self, fieldMeta.self.min, fieldMeta.self.max),
      leadership: parseField(fields.leadership, fieldMeta.leadership.min, fieldMeta.leadership.max),
      exam: parseField(fields.exam, fieldMeta.exam.min, fieldMeta.exam.max),
      portfolio: parseField(fields.portfolio, fieldMeta.portfolio.min, fieldMeta.portfolio.max),
    }

    const biqPoints = parsed.biq.error ? 0 : (parsed.biq.numeric * fieldMeta.biq.weight) / 100
    const surveyPoints = parsed.survey.error ? 0 : (parsed.survey.numeric * fieldMeta.survey.weight) / 100
    const selfPoints = parsed.self.error ? 0 : parsed.self.numeric
    const leadershipPoints = parsed.leadership.error ? 0 : parsed.leadership.numeric
    const examPoints = parsed.exam.error ? 0 : parsed.exam.numeric
    const portfolioPoints = parsed.portfolio.error ? 0 : parsed.portfolio.numeric

    const bonusPoints = bonusIds
      .map((id) => bonusOptions.find((option) => option.id === id)?.points ?? 0)
      .reduce((sum, value) => sum + value, 0)

    const baseTotal = biqPoints + surveyPoints + selfPoints + leadershipPoints + examPoints + portfolioPoints
  ...
  const breakdownItems = useMemo(
    () => [
      ...
      { label: 'Sorğu (15 bal)', value: values.breakdown.surveyPoints, max: 15 },
      { label: 'Özünü dəyərləndirmə', value: values.breakdown.selfPoints, max: 10 },
  ...
  const handleBonusToggle = (id: string) => {
    setBonusIds((prev) => (prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]))
    setShareStatus(null)
  }
  ...
  const handleReset = () => {
    setFields({ biq: '', survey: '', self: '', leadership: '', exam: '', portfolio: '' })
    setBonusIds([])
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Dəyərləri daxil edin, sistem yekun nəticəni hesablayacaq.</p>
        </CardHeader>
            ...
            {(Object.keys(fieldMeta) as FieldKey[]).map((key) => {
              ...
              return (
                <label key={key} className="flex flex-col gap-1 text-sm">
                  <span className="text-xs uppercase tracking-[0.2em] text-muted-foreground">{meta.label}</span>
                  <input
                    className={cn('input', parsed.error && 'border-destructive/60')}
                    type="number"
                    min={meta.min}
                    max={meta.max}
                    value={fields[key]}
                  ...
                  />
                  {parsed.error && <span className="text-xs text-destructive">{parsed.error}</span>}
                </label>
              )
            })}
          </div>
      ...
      <div className="space-y-4">
        <ScoreCard title="Toplam bal" value={values.breakdown.total.toFixed(1)} subtitle="0–100 + bonus" />
