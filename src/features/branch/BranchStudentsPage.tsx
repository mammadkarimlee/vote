...
export const BranchStudentsPage = () => {
  ...
  const loadData = async () => {
    ...
    let studentQuery = supabase.from('students').select('*').eq('org_id', ORG_ID).is('deleted_at', null)
    let groupQuery = supabase.from('groups').select('*').eq('org_id', ORG_ID).is('deleted_at', null)
    studentQuery = studentQuery.eq('branch_id', branchId)
    groupQuery = groupQuery.eq('branch_id', branchId)

    const [studentRes, groupRes] = await Promise.all([studentQuery, groupQuery])

    const groupDocs = (groupRes.data ?? []).map((row) => ({ id: row.id, data: mapGroupRow(row) }))
    setGroups(groupDocs.filter((group) => group.data.branchId === branchId))

    const studentDocs = (studentRes.data ?? []).map((row) => ({ id: row.id, data: mapStudentRow(row) }))
    setStudents(studentDocs.filter((student) => student.data.branchId === branchId))
  }
