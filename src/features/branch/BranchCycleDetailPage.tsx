...
export const BranchCycleDetailPage = () => {
  ...
  const [taskSummaryByStudent, setTaskSummaryByStudent] = useState<Record<string, TaskSummary>>({})
  ...
  useEffect(() => {
    const loadLookups = async () => {
      ...
      if (mappedCycle && mappedCycle.branchIds && mappedCycle.branchIds.length > 0) {
      ...
      setStudents((studentRes.data ?? []).map((row) => ({ id: row.id, data: mapUserRow(row) })))
    }
  ...
  const totals = useMemo(() => {
    ...
    const totalTasks = studentRows.reduce((acc, row) => acc + row.totalCount, 0)
    ...
    <div className="panel">
      ...
      <div className="card">
        ...
        <div className="table">
          ...
          {studentRows.length === 0 && <div className="empty">Məlumat yoxdur.</div>}
        </div>
      </div>
    </div>
  )
