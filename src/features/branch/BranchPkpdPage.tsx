...
const normalizeScale = (value: number, min?: number | null, max?: number | null) => {
  const safeMin = min ?? 1
...
const portfolioLimits = (category?: TeacherCategory) => {
  if (category === 'drama_gym') {
    return { education: 3, attendance: 3, training: 9, olympiad: 20, events: 25 }
  }
  if (category === 'chess') {
    return { education: 3, attendance: 3, training: 9, olympiad: 30, events: 15 }
  }
...
export const BranchPkpdPage = () => {
  ...
  const [biqResults, setBiqResults] = useState<Array<DocEntry<BiqClassResultDoc>>>([])
  const [examResults, setExamResults] = useState<Array<DocEntry<PkpdExamDoc>>>([])
  const [portfolios, setPortfolios] = useState<Array<DocEntry<PkpdPortfolioDoc>>>([])
  const [achievements, setAchievements] = useState<Array<DocEntry<PkpdAchievementDoc>>>([])
  ...
  useEffect(() => {
    const loadLookups = async () => {
      if (!branchId) return
      const [cycleRes, teacherRes, groupRes, subjectRes, assignmentRes] = await Promise.all([
        supabase.from('survey_cycles').select('*').eq('org_id', ORG_ID),
        supabase.from('teachers').select('*').eq('org_id', ORG_ID).is('deleted_at', null).eq('branch_id', branchId),
        supabase.from('groups').select('*').eq('org_id', ORG_ID).is('deleted_at', null).eq('branch_id', branchId),
        supabase.from('subjects').select('*').eq('org_id', ORG_ID).is('deleted_at', null),
        supabase.from('teaching_assignments').select('*').eq('org_id', ORG_ID).eq('branch_id', branchId),
      ])
      ...
      if (visibleCycles.length > 0) {
        const latest = [...visibleCycles].sort((a, b) => b.data.year - a.data.year)[0]
  ...
  useEffect(() => {
    const loadPkpdData = async () => {
      ...
      const [questionRes, taskRes, biqRes, examRes, portfolioRes, achievementRes, decisionRes] = await Promise.all([
        supabase.from('questions').select('*').eq('org_id', ORG_ID),
        supabase.from('tasks').select('*').eq('org_id', ORG_ID).eq('cycle_id', selectedCycleId).eq('branch_id', branchId),
        supabase
      ...
      const taskDocs = (taskRes.data ?? []).map((row) => ({ id: row.id, data: mapTaskRow(row) }))
      setTasks(taskDocs)

      const biqDocs = (biqRes.data ?? []).map((row) => ({ id: row.id, data: mapBiqClassResultRow(row) }))
      setBiqResults(biqDocs)

      const examDocs = (examRes.data ?? []).map((row) => ({ id: row.id, data: mapPkpdExamRow(row) }))
      setExamResults(examDocs)
      setExamDrafts(
        Object.fromEntries(
          examDocs.map((row) => [row.data.teacherId, row.data.score !== null ? String(row.data.score) : '']),
        ),
      )

      const portfolioDocs = (portfolioRes.data ?? []).map((row) => ({ id: row.id, data: mapPkpdPortfolioRow(row) }))
      ...
      setDecisionDrafts(
        decisionDocs.reduce<Record<string, { status: PkpdDecisionStatus; note: string }>>((acc, item) => {
  ...
  const cycle = useMemo(() => cycles.find((item) => item.id === selectedCycleId), [cycles, selectedCycleId])
  ...
  const biqMap = useMemo(
    () => Object.fromEntries(biqResults.map((item) => [`${item.data.groupId}_${item.data.subjectId}`, item.data])),
    [biqResults],
  )

  const portfolioMap = useMemo(
    () => Object.fromEntries(portfolios.map((item) => [item.data.teacherId, item.data])),
    [portfolios],
  )

  const portfolioTeacher = portfolioTeacherId ? teacherMap[portfolioTeacherId] : undefined
  const portfolioMax = portfolioLimits(portfolioTeacher?.category)

  const examMap = useMemo(
    () => Object.fromEntries(examResults.map((item) => [item.data.teacherId, item.data])),
    [examResults],
  )
  ...
  const achievementTotals = useMemo(() => {
    ...
    achievements.forEach((item) => {
      totals[item.data.teacherId] = (totals[item.data.teacherId] ?? 0) + item.data.points
    })
    return totals
  }, [achievements])

  const decisionMap = useMemo(
    () => Object.fromEntries(decisions.map((item) => [item.data.teacherId, item.data])),
    [decisions],
  )
  ...
  const flowStats = useMemo(() => {
    ...
    const stats: Record<
      string,
      { student: { sum: number; count: number }; management: { sum: number; count: number }; self: { sum: number; count: number } }
    > = {}
    ...
    answers.forEach((answer) => {
      ...
      const normalized = normalizeScale(numeric, question.scaleMin, question.scaleMax)
      ...
      } else if (task.raterRole === 'manager' && task.targetType === 'teacher') {
        stats[targetId].management.sum += normalized
        stats[targetId].management.count += 1
      } else if (task.raterRole === 'teacher' && task.targetType === 'teacher') {
  ...
  const standardTeachers = useMemo(
    () => teachers.filter((teacher) => (teacher.data.category ?? 'standard') === 'standard'),
    [teachers],
  )
  ...
  const summaryRows = useMemo(() => {
    return teachers.map((teacher) => {
      ...
      const weights =
        category === 'standard'
          ? { student: 15, management: 10, self: 10, biq: 15, exam: 30, portfolio: 20 }
          : { student: 20, management: 10, self: 10, biq: 0, exam: 0, portfolio: 60 }

      const stats = flowStats[teacher.id]
      const studentAvg = stats && stats.student.count > 0 ? stats.student.sum / stats.student.count : null
      const managementAvg = stats && stats.management.count > 0 ? stats.management.sum / stats.management.count : null
      const selfAvg = stats && stats.self.count > 0 ? stats.self.sum / stats.self.count : null

      const studentScore = studentAvg === null ? null : (studentAvg * weights.student) / 100
      const managementScore = managementAvg === null ? null : (managementAvg * weights.management) / 100
      const selfScore = selfAvg === null ? null : (selfAvg * weights.self) / 100

      const assignmentsForTeacher = assignmentByTeacher[teacher.id] ?? []
      const biqScores = assignmentsForTeacher
        .map((assignment) => biqMap[`${assignment.groupId}_${assignment.subjectId}`]?.score)
        .filter((value): value is number => typeof value === 'number')
      const biqAvg = biqScores.length > 0 ? biqScores.reduce((a, b) => a + b, 0) / biqScores.length : null
      const biqScore = weights.biq === 0 || biqAvg === null ? null : (biqAvg * weights.biq) / 100

      const examScore = weights.exam === 0 ? null : examMap[teacher.id]?.score ?? null
  ...
  }, [achievementTotals, assignmentByTeacher, biqMap, examMap, flowStats, portfolioMap, teachers])
  ...
  const handleSaveBiq = async () => {
    ...
    const { data } = await supabase
      ...
      .eq('cycle_id', selectedCycleId)
      .eq('branch_id', branchId)
    setBiqResults((data ?? []).map((row) => ({ id: row.id, data: mapBiqClassResultRow(row) })))
  }
  ...
  const handleImportBiq = async (file: File) => {
    ...
    rows.forEach((row) => {
      ...
      const subjectId =
        ...
        (subjectRaw ? subjectNameMap.get(subjectRaw.toLowerCase()) ?? null : null)
      if (!subjectId) {
        missingSubject += 1
        return
      }
  ...
  const handleDeleteBiq = async (id: string) => {
    if (!branchId || !selectedCycleId) return
    await supabase.from('biq_class_results').delete().eq('org_id', ORG_ID).eq('id', id)
  ...
  const handleSaveExam = async (teacherId: string) => {
    ...
    if (!raw || raw.trim() === '') {
      ...
      setExamResults((prev) => prev.filter((item) => item.data.teacherId !== teacherId))
      return
    }
    ...
    const { data } = await supabase
      ...
      .eq('cycle_id', selectedCycleId)
      .eq('branch_id', branchId)
    setExamResults((data ?? []).map((row) => ({ id: row.id, data: mapPkpdExamRow(row) })))
  }
  ...
  const handleSavePortfolio = async () => {
    ...
    const { data } = await supabase
      ...
      .eq('cycle_id', selectedCycleId)
      .eq('branch_id', branchId)
    setPortfolios((data ?? []).map((row) => ({ id: row.id, data: mapPkpdPortfolioRow(row) })))
  ...
  const handleAddAchievement = async () => {
    ...
    const { data } = await supabase
      ...
      .eq('cycle_id', selectedCycleId)
      .eq('branch_id', branchId)
    setAchievements((data ?? []).map((row) => ({ id: row.id, data: mapPkpdAchievementRow(row) })))
    setStatus('Bonus əlavə edildi')
  }

  const handleDeleteAchievement = async (id: string) => {
    await supabase.from('pkpd_achievements').delete().eq('org_id', ORG_ID).eq('id', id)
  ...
  const handleSaveDecision = async (teacherId: string) => {
    ...
    const { data } = await supabase
      ...
      .eq('cycle_id', selectedCycleId)
      .eq('branch_id', branchId)
    setDecisions((data ?? []).map((row) => ({ id: row.id, data: mapPkpdDecisionRow(row) })))
    setStatus('Qərar saxlanıldı')
  }

  return (
    <div className="panel">
      {isSuperAdmin && <BranchSelector branchId={branchId} branches={branches} onChange={setBranchId} />}
      ...
      <div className="panel-header">
        ...
        <div className="actions">
          <label className="field">
            ...
            <select className="input" value={selectedCycleId} onChange={(event) => setSelectedCycleId(event.target.value)}>
      ...
      <div className="card">
        ...
        <div className="form-row">
          <select className="input" value={biqGroupId} onChange={(event) => setBiqGroupId(event.target.value)}>
          ...
          <select className="input" value={biqSubjectId} onChange={(event) => setBiqSubjectId(event.target.value)}>
        ...
        <div className="form-row">
          ...
          />
          <button className="btn primary" type="button" onClick={handleSaveBiq} disabled={!selectedCycleId}>
            Saxla
          </button>
        </div>
        <div className="form-row">
        ...
        <div className="data-table">
          ...
          {standardTeachers.map((teacher) => (
            <div className="data-row" key={teacher.id}>
              ...
              <div className="actions">
                <button className="btn" type="button" onClick={() => void handleSaveExam(teacher.id)}>
                  Saxla
                </button>
              </div>
            </div>
        ...
        {portfolioTeacherId && (
          <>
            ...
            <div className="form-row">
              ...
              />
              <button className="btn primary" type="button" onClick={handleSavePortfolio}>
                Saxla
              </button>
            </div>
          </>
      ...
      <div className="card">
        ...
        <div className="form-row">
          ...
          />
          <button className="btn primary" type="button" onClick={handleAddAchievement}>
            Əlavə et
          </button>
        </div>
        <div className="data-table">
          ...
          {achievements.map((item) => (
            <div className="data-row" key={item.id}>
              ...
              <div>{formatShortDate(item.data.createdAt)}</div>
              <div className="actions">
                <button className="btn ghost" type="button" onClick={() => void handleDeleteAchievement(item.id)}>
                  Sil
                </button>
              </div>
            </div>
          ...
          {summaryRows.map((row) => (
            <div className="data-row" key={row.teacherId}>
              ...
              <div>
                <select
                  className="input"
                  value={decisionDrafts[row.teacherId]?.status ?? decisionMap[row.teacherId]?.status ?? 'PENDING'}
                  onChange={(event) =>
                    setDecisionDrafts((prev) => ({
                      ...prev,
                      [row.teacherId]: {
                        status: event.target.value as PkpdDecisionStatus,
                        note: prev[row.teacherId]?.note ?? decisionMap[row.teacherId]?.note ?? '',
                      },
                    }))
                  }
                >
              ...
              <div>
                <input
                  className="input"
                  placeholder="Qeyd"
                  value={decisionDrafts[row.teacherId]?.note ?? decisionMap[row.teacherId]?.note ?? ''}
                  onChange={(event) =>
                    setDecisionDrafts((prev) => ({
                      ...prev,
                      [row.teacherId]: {
                        status: prev[row.teacherId]?.status ?? decisionMap[row.teacherId]?.status ?? 'PENDING',
              ...
              <div className="actions">
                <button className="btn" type="button" onClick={() => void handleSaveDecision(row.teacherId)}>
                  Saxla
                </button>
              </div>
            </div>
