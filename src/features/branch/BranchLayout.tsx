...
const NavTile = ({
	...
	<NavLink
		to={to}
		className={({ isActive }) => cn("icon-tile", isActive && "active")}
	>
...
const MenuGroup = ({ title, items }: NavGroup) => (
...
export const BranchLayout = ({ isAdmin = false }: { isAdmin?: boolean }) => {
	...
	const branchGroups: NavGroup[] = [
		...
		},
		{
			...
			items: [
				{
					to: "/branch/assignments",
					label: "Dərs təyinatları",
					icon: "assignments",
				},
				{
