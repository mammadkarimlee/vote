...
const PROVISION_API_URL = import.meta.env.VITE_PROVISION_API_URL || 'http://localhost:8787'
...
export const provisionLoginUser = async (params: {
  ...
  if (!response.ok) {
    throw await extractFunctionError(new Error(`HTTP ${response.status}`), payload)
  }

  return payload as { uid: string; login: string; password: string; email: string }
}
