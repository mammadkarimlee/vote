...
export const BranchSubjectsPage = () => {
  ...
  const loadSubjects = async () => {
    const { data, error } = await supabase
      ...
      .is('deleted_at', null)
    if (error) return
    const items = (data ?? []).map((row) => ({ id: row.id, data: mapSubjectRow(row) }))
    setSubjects(items)
  }
  ...
  const handleImport = async (file: File) => {
    ...
    const existing = new Set(subjects.map((subject) => subject.data.name.toLowerCase()))
    ...
    setStatus(`Bulk import tamamlandı. Missing: ${missing}, Duplicate: ${duplicates}`)
    await loadSubjects()
  }
          ...
          <div className="data-row" key={subject.id}>
            ...
            <div className="actions">
              {editingId === subject.id ? (
                ...
                <>
                  <button className="btn" type="button" onClick={() => handleEditStart(subject)}>
                    Redaktə
                  </button>
                  <button className="btn ghost" type="button" onClick={() => void handleDelete(subject.id)}>
                    Sil
                  </button>
                </>
              )}
