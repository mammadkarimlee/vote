...
export const BranchGroupsPage = () => {
  ...
  const loadGroups = async () => {
    ...
    let query = supabase.from('groups').select('*').eq('org_id', ORG_ID).is('deleted_at', null)
    query = query.eq('branch_id', branchId)

    const { data, error } = await query
    if (error) return
    const items = (data ?? []).map((row) => ({ id: row.id, data: mapGroupRow(row) }))
    setGroups(items.filter((group) => group.data.branchId === branchId))
  }

  const loadBranchName = async () => {
    if (!branchId) return
    const { data } = await supabase.from('branches').select('name').eq('org_id', ORG_ID).eq('id', branchId).maybeSingle()
  ...
  const handleImport = async (file: File) => {
    ...
    const existingNames = new Set(groups.map((group) => group.data.name.toLowerCase()))
    ...
    <div className="panel">
      ...
      <div className="card">
        ...
        <div className="form-row">
          ...
          />
          <span className="hint">Şablon sütunları: name, classLevel, branchId (optional)</span>
        </div>
          ...
          <div className="data-row" key={group.id}>
            ...
            <div className="actions">
              {editingId === group.id ? (
                ...
                <>
                  <button className="btn" type="button" onClick={() => handleEditStart(group)}>
                    Redaktə
                  </button>
                  <button className="btn ghost" type="button" onClick={() => void handleDelete(group.id)}>
                    Sil
                  </button>
                </>
              )}
