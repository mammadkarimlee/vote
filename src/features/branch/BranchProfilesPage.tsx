...
export const BranchProfilesPage = () => {
  ...
  const loadUsers = async () => {
    ...
    const { data, error } = await supabase
      ...
      .is('deleted_at', null)
    if (error) return
    const items = (data ?? []).map((row) => ({ id: row.id, data: mapUserRow(row) }))
    setUsers(items)
  }
  ...
  return (
    <div className="panel">
      ...
      <div className="card">
        ...
        {role === 'manager' && <div className="hint">Rəhbərlik üçün şifrə login ilə eynidir.</div>}
      ...
      <div className="table">
        ...
        {users.map((userRow) => (
          <div className="table-row" key={userRow.id}>
            ...
            <div>
              <button className="btn ghost" type="button" onClick={() => void handleDelete(userRow.id)}>
                Sil
              </button>
            </div>
          </div>
