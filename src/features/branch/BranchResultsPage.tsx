...
export const BranchResultsPage = () => {
	...
	useEffect(() => {
		const loadLookups = async () => {
			...
			const [cycleRes, teacherRes, groupRes, subjectRes, questionRes] =
				await Promise.all([
			...
			const allCycles = (cycleRes.data ?? []).map((row) => ({
				id: row.id,
			...
			if (cycleDocs.length > 0) {
				...
				if (
					!selectedCycleId ||
					!cycleDocs.some((cycle) => cycle.id === selectedCycleId)
				) {
