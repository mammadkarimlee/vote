...
import { mapManagementAssignmentRow, mapUserRow } from '../../lib/supabaseMappers'
...
export const BranchManagementAssignmentsPage = () => {
  ...
  const [managers, setManagers] = useState<Array<{ id: string; data: UserDoc }>>([])
  const [assignments, setAssignments] = useState<Array<{ id: string; data: ManagementAssignmentDoc }>>([])
  ...
  const loadData = async () => {
    ...
    const [usersRes, assignmentsRes, branchRes] = await Promise.all([
      ...
      supabase
        ...
        .eq('branch_id', branchId)
        .is('deleted_at', null),
      supabase.from('branches').select('name').eq('org_id', ORG_ID).eq('id', branchId).maybeSingle(),
    ])

    const managerUsers = (usersRes.data ?? []).map((row) => ({ id: row.id, data: mapUserRow(row) }))
    ...
    <div className="panel">
      ...
      <div className="card">
        ...
        <div className="form-grid">
          <select className="input" value={managerUid} onChange={(event) => setManagerUid(event.target.value)}>
