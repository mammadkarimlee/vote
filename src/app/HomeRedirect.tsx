...
const getLastPath = (role: string) => {
  ...
  if (role === 'superadmin') {
    return stored.startsWith('/admin') || stored.startsWith('/branch') || stored.startsWith('/pkpd') || stored.startsWith('/me')
      ? stored
      : null
  }
  if (role === 'branch_admin' || role === 'moderator') {
    return stored.startsWith('/branch') || stored.startsWith('/pkpd') || stored.startsWith('/me') ? stored : null
  }
  if (role === 'student' || role === 'teacher' || role === 'manager') {
    return stored.startsWith('/vote') || stored.startsWith('/me') ? stored : null
  }
  return null
}
